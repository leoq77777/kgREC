...
def train_model(graph_path, model_save_path, epochs=10, batch_size=256, embedding_dim=64, model_type='mf'):
    # 数据预处理
    # 获取图数据
    ...
    if model_type == 'lightgcn':
        # 根据图中总节点数调整num_items
        total_nodes = max_user_id + 1 + max_movie_id + 1  # 用户+电影节点总数
        model = LightGCN(num_users + 1, total_nodes - (num_users + 1), embedding_dim).to(device)
        ...
        model = KnowledgeEnhancedMF(num_users+1, num_items+1, embedding_dim, kg_embeddings).to(device)
    ...
    if model_type == 'dual_tower':
        # 确保特征数据正确准备
        # 特征预处理 (实际项目中需要实现这些函数)
        user_features = np.array([np.random.rand(10) for _ in range(len(ratings))]) if user_features is None else user_features
        item_features = np.array([np.random.rand(10) for _ in range(len(item_ids))])  # 确保item_features始终被赋值
        labels = np.random.randint(0, 2, size=len(ratings)) if labels is None else labels
        # 移除重复的特征定义
        labels = np.random.randint(0, 2, size=len(ratings)) if labels is None else labels
    ...
    if model_type == 'dual_tower':
        ...
        model = train_dual_tower(model, train_dataset, val_dataset, epochs=epochs, batch_size=batch_size)
    ...
    # 初始化特征变量（所有模型类型通用）
    user_features = np.array([np.random.rand(10) for _ in range(len(ratings))])
    item_features = np.array([np.random.rand(10) for _ in range(len(item_ids))])
    labels = np.random.randint(0, 2, size=len(ratings))
...
if __name__ == '__main__':
    ...
    MODEL_SAVE_PATH = 'models/kg_rec_mf_small.pth'  # 小型模型保存路径
    
    print('开始使用小型数据集训练模型...')
    # 训练LightGCN模型
    model = train_model(GRAPH_PATH, 'models/lightgcn_model.pth', epochs=3, batch_size=2, embedding_dim=2, model_type='lightgcn')
    print('LightGCN模型训练完成!')

    # 训练双塔模型
    print('开始训练双塔模型...')
    model = train_model(GRAPH_PATH, 'models/dual_tower_model.pth', epochs=10, batch_size=64, embedding_dim=32, model_type='dual_tower')
    print('双塔模型训练完成!')
    
    # 训练原有MF模型进行对比
    model = train_model(GRAPH_PATH, MODEL_SAVE_PATH, epochs=5, batch_size=128)  # 减少epochs和batch_size加速测试
    print('小型数据集训练完成!')